...
- 舱内可拾取物使用 `Pickup` Layer。
- `Pickup` 与 `Floor`、`SubmarineInterior`、`Player` 和其他 `Pickup` 保持碰撞。
- `Pickup` 与 `Submarine`、`Anchor`、`AnchorLauncher`、`FloatingItem` 互相忽略。
- Pickup Prefab 同时保留实体 Collider 和拾取 Trigger：实体 Collider 承接重力并停在地板上，Trigger 继续服务 `PlayerCarryInteractor2D`。
- Pickup 使用 Dynamic `Rigidbody2D`，但不作为潜艇或 `SubmarineInteriorFollower2D` 的 Transform 子物体。
- 重力和碰撞只作用于独立的 Kinematic 舱内代理，不向潜艇主体的 Dynamic `Rigidbody2D` 传递反作用力。
- 锚拉回物品后，由 `SubmarineInteriorFollower2D` 在圆形舱室内随机采样空位，并避开玩家、地板、舱壁、梯子、操作区和已有 Pickup。
